...
void display_author_books(char *), display_publisher_books(char *),
    display_books();
...
int main() {
  ...
  while (1) {
    ...
    scanf("%s %s %s %f", books[i].book_title, books[i].book_author,
          books[i].book_publisher, &books[i].cost);
    i++, anumber++;
  }
  ...
  for (i = 0; i < 100; i++) {
    if (books[i].accession_number >= 1000) {
      if (strcmp(books[i].book_author, name) == 0)
        printf("%s %s %s %f\n", books[i].book_title, books[i].book_author,
               books[i].book_publisher, books[i].cost);
